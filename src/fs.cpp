...
namespace fs
{
	list_dirs_res list_dirs(const char *dir_filter)
	{
		...
		HANDLE entry = FindFirstFileExW(wdir, FindExInfoBasic, &entry_data, FindExSearchNameMatch, nullptr, 0);
		...
		do
		{
			if (entry_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY && wcscmp(entry_data.cFileName, L".") != 0 && wcscmp(entry_data.cFileName, L"..") != 0)
			{
				++dirs_count;
			}
		}
		...
		uint32_t i {0};
		entry = FindFirstFileExW(wdir, FindExInfoBasic, &entry_data, FindExSearchNameMatch, nullptr, 0);
		do
		{
			if (entry_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY && wcscmp(entry_data.cFileName, L".") != 0 && wcscmp(entry_data.cFileName, L"..") != 0)
			{
	...
	list_files_res list_files(const char* dir_filter, const char* file_filter)
	{
		...
		do
		{
			...
			if (!(entry_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && str::ends_with(fn, file_filter))
			{
				++files_count;
			}
		}
